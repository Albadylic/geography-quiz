...
function buildFlagAsset(iso2: string, id: string): FlagAsset {
  ...
  const source = join(FLAG_SRC, variant, `${code}.svg`);
  ...
  try {
    ...
    throw new Error(
      `No flag asset for ${id} (expected ${variant}/${code}.svg). Source it by hand and record the licence in public/flags/CREDITS.md — do not substitute a placeholder.`,
    );
  }

  const viewBox = /viewBox="([\d.\s-]+)"/.exec(svg)?.[1];
  if (!viewBox) throw new Error(`Flag ${code}.svg has no viewBox; cannot derive a ratio`);
  const parts = viewBox.trim().split(/\s+/).map(Number);
  const [width, height] = [parts[2], parts[3]];
  if (!width || !height) throw new Error(`Flag ${code}.svg has a degenerate viewBox`);
